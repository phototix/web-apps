...
-- Run this file to add WhatsApp functionality to your ERP.ezy.chat

-- Backup existing users table structure
CREATE TABLE IF NOT EXISTS users_backup_whatsapp LIKE users;
INSERT INTO users_backup_whatsapp SELECT * FROM users;

-- 1. Add new columns to users table
ALTER TABLE users 
ADD COLUMN tier ENUM('basic', 'business', 'enterprise') DEFAULT 'basic',
ADD COLUMN invited_by BIGINT UNSIGNED NULL,
ADD COLUMN max_sessions INT DEFAULT 1,
...
-- 3. Create WhatsApp groups table
...
-- 4. Create group messages table (compressed for storage efficiency)
...
-- 5. Create webhook events queue table
...
-- 6. Create real-time updates table (for long polling)
...
-- 8. Update existing users with default tier values
...
-- 9. Create default admin user if none exists
...
SELECT 'WhatsApp database migration completed successfully!' as message;